    ...
    def generate_yaml_template(self, indent_spacing=2, width=78, visibility=0):
        ...
        for lvl, val, obj in data:
            ...
            level_info[lvl]['data'].append(
                ( len(val), len(obj._description or "") ) )
        for lvl in sorted(level_info):
            offset = lvl*indent_spacing + \
                max(val for val,doc in level_info[lvl]['data']) \
                + len(comment)
            over = sum( 1 for val,doc in level_info[lvl]['data']
                        if doc + offset > width )
            if len(level_info[lvl]['data']) - over > 0:
                line = max( offset + doc for val,doc in level_info[lvl]['data']
                            if doc + offset <= width )
            else:
                line = width
        ...
        for lvl, val, obj in data:
            ...
            os.write(comment)
            txtArea = max(width-field-len(comment), 20)
            os.write( ("\n"+' '*field+comment).join( 
